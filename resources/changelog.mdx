...
* `asc web review iaps attach` can now attach non-renewing in-app purchases to App Review submissions through the web-session workflow, matching the subscription review attach surface for IAPs that are not auto-renewable
* The README now includes a macOS TestFlight distribution example for uploading `.pkg` builds and adding them to macOS tester groups
* Community surfaces keep growing with PodSized: AI Podcast Summaries

**Changes**:

* `feat(web)`: Add web-session non-renewing IAP review attach with app-scoped lookup, idempotent already-attached handling, confirmation guardrails, and CLI/network regression coverage
* `docs(testflight)`: Add a macOS TestFlight distribution example to the README
* `build(community)`: Add PodSized: AI Podcast Summaries to the Wall of Apps

### 1.5.2

**Highlights**:

* `asc status --platform MAC_OS` can now scope release dashboards to iOS, macOS, tvOS, or visionOS so universal apps report the correct builds, App Store version, and review submission state
* Community surfaces keep growing with UrbanAir: Unlock all escooters

**Changes**:

* `feat(status)`: Add platform filtering across builds, App Store versions, review submissions, and status summaries
* `build(community)`: Add UrbanAir: Unlock all escooters to the Wall of Apps

### 1.5.1

**Highlights**:

* `asc subscriptions pricing equalize` can now schedule equalized subscription prices with safer start-date handling, dry-run previews, and explicit boolean validation
* Subscription offer-code exports now include cleaner table/markdown output and one-time-code value export coverage for automation-friendly campaign workflows
* Community surfaces keep growing with Sam Webpage Summarizer, AcheNote - Symptom & Visit Log, Allergy Translate, Parrot - AI Personas, Hedwig - Block Text Messages, Squeaker - Dog Training Sounds, Embers to Dyson, Metrociego, ReaderFlow | Read Later, FeedFlow - RSS Feed Reader, Kavale - Securite Equestre, and FishLogger

**Changes**:

* `feat(subscriptions)`: Add scheduled equalized subscription pricing with auto-start-date support, dry-run state reporting, and regression coverage for invalid flag values
* `feat(offer-codes)`: Improve subscription offer-code list/value exports with sanitized names, CSV write-failure coverage, and canonical command examples
* `build(community)`: Add twelve new apps to the Wall of Apps
...
* `asc background-assets submit` can now submit background asset versions for App Review, with dry-run planning, explicit version selection, review submission reuse, and safer attach rollback behavior
* `asc apps wall submit --country jp` supports non-US storefront lookups, so community submissions can resolve App Store links like Japan-only apps without manual URL work
* Metadata validation preserves blank template behavior while tightening app-info/version-localization checks
* Community surfaces keep growing with Fablum: EPUB & PDF Reader, PronoGP, and Steplee – Love. Walk. Win.

**Changes**:

* `feat(background-assets)`: Add background asset submit-for-review command with CLI and network regression coverage
* `fix(apps)`: Add `--country` to `apps wall submit` for non-US App Store storefront lookups
* `fix(metadata)`: Preserve blank template validation behavior while addressing review feedback on metadata checks
* `build(community)`: Add three new apps to the Wall of Apps

### 1.4.1

**Highlights**:

* `asc metadata init` can now scaffold App Store metadata templates with locale-aware files and validation-friendly defaults
* Subscriptions add an experimental guarded Monthly with 12-Month Commitment command surface for Apple's April 2026 subscription update while the public API lacks billing-mode fields
* Xcode Cloud secret environment variables now fetch the client encryption key from the correct CI auth endpoint, fixing secret creation failures
* Community surfaces keep growing with Fitnit: Health & Fitness, Spotter - The Travel Companion, and Greenie: Plant Identifier AI

**Changes**:

* `feat(subscriptions)`: Add an experimental guarded Monthly with 12-Month Commitment command surface for Apple's April 27, 2026, subscription announcement (https://developer.apple.com/news/?id=agq42lxe), App Store Connect help docs (https://developer.apple.com/help/app-store-connect/manage-subscriptions/set-availability-for-an-auto-renewable-subscription/), and third-party coverage (https://9to5mac.com/2026/04/27/apple-introduces-monthly-subscriptions-with-12-month-commitment-on-the-app-store/), with client-side validation for ONE_YEAR subscriptions, excluded USA/Singapore territories, and the 1.5x upfront-price rule while the public App Store Connect API lacks billing-mode fields.
* `feat(metadata)`: Add metadata init templates for app-info and version localizations
* `fix(xcode-cloud)`: Fetch the secret environment-variable encryption key from the CI auth endpoint outside the `/ci/api` base path
* `docs(metadata)`: Clarify when version localization IDs are available for `asc metadata validate`
* `build(community)`: Add three new apps to the Wall of Apps
...
* `asc capabilities` adds a built-in coverage report for high-value App Store Connect workflows, with JSON, table, and markdown output plus `--status` and `--area` filters
* `asc publish appstore --metadata-dir metadata` can now apply version metadata before App Store submission, making the one-shot publish flow safer for prepared metadata directories
* Subscription and monetization workflows are stronger: free-trial offer codes no longer require prices, subscription review notes can be updated, and experimental web-session subscription availability removal is available with confirmation and verification guardrails
* Analytics report relationship links now accept Apple-prefixed report IDs, and metadata locale handling includes the current Bangla `bn-BD` App Store locale
* Community surfaces keep growing with Kilowatt, Floating Head, Pocket Mole Watch, Master Indicator, Margin, Sift, ZenJournal, MealMate, and DayCount

**Changes**:

* `feat(capabilities)`: Add `asc capabilities` for CLI, API, web-only, and public-API-limited workflow coverage reporting
* `feat(publish)`: Add App Store publish metadata directory support with validation, dry-run planning, and version metadata application before submission
* `feat(subscriptions)`: Add subscription review note updates and experimental web subscription remove-from-sale support
* `fix(offer-codes)`: Allow `FREE_TRIAL` subscription offer codes without price payloads and reject incompatible price inputs
* `fix(analytics)`: Accept prefixed analytics report IDs when fetching report relationship links
* `fix(metadata)`: Add the Bangla `bn-BD` locale alias for App Store metadata workflows
* `build(ci)`: Run govulncheck with a patched Go toolchain and refresh Go formatting/tooling dependencies
* `build(community)`: Add nine new apps to the Wall of Apps
